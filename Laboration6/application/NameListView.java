...
public class NameListView extends BorderPane {
	...
	/**
	 * Creates the list view for the names.Also creates buttons for
	 * adding/removing names/numbers.
	 * 
	 * @param phoneBook
	 *            the phone book with names and phone numbers
	 */
	public NameListView(PhoneBook phoneBook) {
		...
		// The list view is automatically updated when the observable list i
		// updated.
		...
		removeButton.setOnAction(e -> removeButton());

		removeNumberButton = new Button("Remove number");
		removeNumberButton.setOnAction(e -> removeNumberButton());
	 ...
	 * 
	 * @param col
	 *            a collection containing strings that will be displayed in the
	 *            list view
	 */
	...
	private void add() {
		...
		Optional<String[]> result = Dialogs.twoInputsDialog("Add phone book entry", "Enter the name and phone number and press Ok.", labels);
		...
		if (inputs.length>0) {
			...
			if (inputs.length==1){
				boolean success = phoneBook.put(inputs[0], null);
				if (success) {
					obsList.setAll(phoneBook.names());
					select(inputs[0]);
				} else {
	...
	private void addNumber() {
		...
		if (index != -1) {
			...
			Optional<String> result = Dialogs.oneInputDialog("Add phone number to " + name, "Enter the number to add",
					"Number");
			if (result.isPresent()) {
	...
	private void removeNumberButton() {
		int index = listView.getSelectionModel().getSelectedIndex();
		if (index != -1) {
			String name = obsList.get(index);
			Optional<String> result = Dialogs.oneInputDialog("Remove number", "Enter the number to remove and press ok",
					"Number");
			if (result.isPresent()) {
				String input = result.get();
				...
				if (success) {
					...
					Dialogs.alert("Remove", null, "Failed to remove phone number.");
				}
			}
		}
	}
	...
	private void removeButton() {
		if (Dialogs.confirmDialog("Remove", "Y/N", "This will remove the selected element")) {
			int index = listView.getSelectionModel().getSelectedIndex();
			if (index != -1) {
				String name = obsList.get(index);
				if (name != null) {
					boolean success = phoneBook.remove(name);
					if (success) {
						select(index);
						obsList.setAll(phoneBook.names());
					} else {
						Dialogs.alert("Remove", null, "Failed to remove entry.");
					}
				}
			}
		}
