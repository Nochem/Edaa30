...
public class SimpleHashMap<K, V> implements Map<K, V> {
	...
	public V put(K key, V value) {
		...
		while (current != null) {
			if (current.value.equals(newEntry.value)) {
				return value;// current.setValue(newEntry.value);
			}
			previous = current;
	...
	/**
	 * Removes the element with Key key from the map. * Returns removed value if
	 * success, null if there is no such key
	 */
	@Override
	public V remove(Object key) {
		...
		if (table[index] != null) {
			if (table[index].key.equals(kKey)) {
				table[index]=null;
				size--;
	...
	/**
	 * Removes the element with Value value from the map. * Returns removed
	 * value if success, null if there is no such value
	 */
	public V removeValue(Object key, Object value) {
		if (size == 0) {
			return null;
		}
		K kKey = (K) key;
		V vValue = (V) value;
		int index = index(kKey);
		Entry<K, V> current = table[index], previous;
		if (table[index] != null) {
			if (table[index].value.toString().equals(vValue.toString())) {
			...
			while (current.next != null) {
				previous = current;
				current = current.next;
				if (current.value.toString().equals(vValue.toString())) {
	...
	private int index(K key) {
		int index = Math.abs((key.toString().hashCode())) % (int) (capacity - 1);
		return index;
	}

	public Set<String> findNumbers(String name) {
		Set<String> set = new HashSet<String>();
		int index = index((K) name);
		Entry<K, V> current = table[index];
		while (current != null) {
			set.add(current.value.toString());
			current = current.next;
		}
		return set;
	}
	...
	public Set<String> findNames(String number) {
		Set<String> set = new HashSet<String>();
		for (int i=0; i < (capacity-1); i++) { //capacity-1 might be wrong
			Entry<K, V> tempEntry = table[i];
			if (table[i] != null) {
				if (table[i].value.toString().equals(number)) {
					set.add(table[i].key.toString());
				} else {
					while (tempEntry.next != null) {
						if (tempEntry.next.value.toString().equals(number)) {
							set.add(tempEntry.next.key.toString());
						}
						tempEntry = tempEntry.next;
					}
				}
	...
	public Set<String> names() {
		Set<String> set = new HashSet<String>();
		for (int i=0; i < capacity; i++) { //capacity-1 might be wrong
			if (table[i] != null) {
				set.add(table[i].key.toString());
			}
		}
